...
export function getSortedTimeline(): TimelineItem[] {
  return [...TIMELINE_ITEMS].sort(
    (a, b) =>
      parseYearMonth(b.date).date.getTime() -
      parseYearMonth(a.date).date.getTime()
  );
}

export function getRecentTimeline(count: number): TimelineItem[] {
  return getSortedTimeline().slice(0, count);
}

export function getTimelineByYear(): Record<string, TimelineItem[]> {
  return getSortedTimeline().reduce<Record<string, TimelineItem[]>>(
    (acc, item) => {
